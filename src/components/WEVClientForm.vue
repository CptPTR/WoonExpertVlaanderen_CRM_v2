...
const isClientDuplicate = computed(() => {
    return klantenStore.klanten.filter((klant: Klant) =>
        klant.voornaam === form.value.voornaam &&
        klant.achternaam === form.value.familienaam &&
        klant.emailadres === form.value.emailadres &&
...
const selectKlant = (id: string) => {
    ...
    // if (form.value.facturatie_bestemming === FacturatieBestemming.HETZELFDE) {
    //     form.value.facturatieID = id
    // }
}

const addClient = async () => {
    const { data: uploadedKlant } = await supabase.from('klanten').insert([{
        ...
        <div class="content">
            <div class="form" style="width: 325px;">
                <ul>
                    ...
                    <li>
                        <InputText type="text" v-model="form.telefoonnummer" placeholder="Telefoonnummer"
                            id="telefoonnummer" />
                    </li>
                </ul>
                <div class="acties">
                    <input type="button" value="Toevoegen" class="confirm" @click="addClient"
                        :disabled="isClientDuplicate || !form.voornaam || !form.familienaam || !form.emailadres || !form.telefoonnummer" />
                </div>
            </div>
            <ClientList @selectKlant="selectKlant" :selectedKlant="form.klantID"
