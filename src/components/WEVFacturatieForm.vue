...
const isFacturatieDuplicate = computed(() => {
    return facturatiesStore.facturaties.filter((fac: Facturatie) =>
        ...
        fac.vlaamse_stad.id === keuringFacturatie.value.vlaamse_stad.id
    ).length !== 0
})
...
const addFacturatie = async () => {
    const { data: uploadedFacturatie } = await supabase.from('facturaties').insert([{
        ...
        organisatieID: null,
        vlaamse_stad_ID: keuringFacturatie.value.vlaamse_stad.id
    }]).select("*, organisatie: organisaties(*), vlaamse_stad: vlaamse_steden(*)")
        ...
        <div class="facturatie">
            <div class="content">
                <div class="form-bestemming">
                    <!-- <ul v-if="form.facturatie.bestemming === FacturatieBestemming.ANDERS && form.facturatie"> -->
                    <ul>
                        <li>
                            <InputText type="text" v-model="keuringFacturatie.voornaam" id="fac_voornaam"
                                placeholder="Voornaam" required />
                            <InputText type="text" v-model="keuringFacturatie.achternaam" id="fac_familienaam"
                                placeholder="Familienaam" required />
                        </li>
                        <li>
                            <InputText type="email" v-model="keuringFacturatie.emailadres" id="fac_emailadres"
                                placeholder="Emailadres" required />
                        </li>
                        <li>
                            <InputText type="tel" v-model="keuringFacturatie.telefoonnummer" id="fac_telefoonnummer"
                                pattern="0*([1-9][0-9]*)(?:\s?\d{2}){3}" placeholder="Telefoonnummer" required />
                        </li>
                        <li>
                            <InputText type="text" v-model="keuringFacturatie.straatnaam" id="fac_straatnaam"
                                placeholder="Straat" required />
                            <InputText type="text" v-model="keuringFacturatie.nummer" id="fac_nummer" placeholder="Nr"
                                maxlength="10" required />
                        </li>
                        <li v-if="keuringFacturatie.vlaamse_stad">
                            <Dropdown resetFilterOnHide filter v-model="keuringFacturatie.vlaamse_stad"
                    ...
                    <div class="acties" v-if="form.facturatie_bestemming === FacturatieBestemming.ANDERS">
                        <input type="button" value="Toevoegen" class="confirm" :disabled="isFacturatieDuplicate ||
                            ...
                            !keuringFacturatie.vlaamse_stad" @click="addFacturatie" />
                    </div>

                </div>
                <FacturatieList @select-facturatie="(id) => selectFacturatie(id)" @selectFacturatie="selectFacturatie"
